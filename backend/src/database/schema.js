const { pgTable, uuid, varchar, date, time, integer, timestamp, boolean, text, jsonb } = require('drizzle-orm/pg-core');
...
const absenceEntries = pgTable('absence_entries', {
  ...
  userId: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  entryDate: date('entry_date').notNull(),
  reasonId: uuid('reason_id').references(() => absenceReasons.id, { onDelete: 'set null' }),
  notes: text('notes'),
