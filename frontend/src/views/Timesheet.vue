      ...
      <form @submit.prevent="handleSubmit" class="space-y-4 md:space-y-5">
        ...
        <div>
          ...
          <textarea
            ...
            class="form-input resize-none overflow-hidden min-h-[80px]"
            @paste="handlePaste"
            @input="autoResize($event.target)"
          ></textarea>
          <div
            v-if="parseToast.show"
            class="mt-1.5 flex items-center gap-2 p-2 glass rounded-lg border-l-2 border-l-accent"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
            ...
            <span class="text-xs text-accent flex-1">{{
              parseToast.message
            }}</span>
            <button
              @click="undoParse"
              type="button"
              class="text-xs text-accent font-medium hover:underline flex-shrink-0"
            >
              Batal
            </button>
          </div>
...
watch(
  ...
  () => {
    nextTick(() => {
      if (rincianTextarea.value) {
        autoResize(rincianTextarea.value);
      }
    });
  },
);
