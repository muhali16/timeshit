  ...
  <div class="space-y-5 md:space-y-0 md:grid md:grid-cols-12 md:gap-6">
    ...
    <div class="md:col-span-4 lg:col-span-3 space-y-5">
      ...
      <div class="glass glass-hover rounded-2xl p-5 md:p-6 text-center">
        <div class="relative inline-block mb-3">
          ...
          >
            <span class="text-accent font-bold text-2xl md:text-3xl">{{ (authStore.user?.name || '?')[0]?.toUpperCase() }}</span>
          </div>
          <div class="absolute -bottom-1 -right-1 w-6 h-6 rounded-full bg-success border-2 border-bg-primary flex items-center justify-center">
        ...
        <p class="font-bold text-text-primary text-lg">{{ authStore.user?.name }}</p>
        <p class="text-text-tertiary text-xs mt-0.5 truncate">{{ authStore.user?.email }}</p>
      </div>
      ...
      <div class="glass rounded-2xl p-5 space-y-4">
        <div class="flex items-center justify-between">
          ...
          <span class="text-text-primary text-xs font-medium">{{ form.timezone || 'Asia/Jakarta' }}</span>
        </div>
        <div class="flex items-center justify-between">
          <span class="text-text-tertiary text-xs font-medium">Lokasi tersimpan</span>
          <span class="text-accent text-xs font-medium">{{ form.locations.length }}</span>
        </div>
        <div class="flex items-center justify-between">
          <span class="text-text-tertiary text-xs font-medium">Member sejak</span>
          <span class="text-text-primary text-xs font-medium">{{ memberSince }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="md:col-span-8 lg:col-span-9 space-y-5">
      <div class="mb-2 md:mb-4">
        <h1 class="text-xl md:text-2xl font-bold text-text-primary tracking-tight">Settings</h1>
      ...
      <div class="glass rounded-2xl p-5 md:p-6">
        <h2 class="text-sm md:text-base font-semibold text-text-primary mb-4 flex items-center gap-2">
        ...
        <div class="space-y-3.5 md:space-y-4 md:grid md:grid-cols-2 md:gap-4">
          <div class="md:col-span-2">
            <label class="block text-xs font-medium text-text-secondary mb-1.5">Nama</label>
          ...
          <div class="relative" ref="tzWrapper">
            ...
            <div v-if="tzOpen" class="absolute z-30 top-full left-0 right-0 mt-1 glass-strong rounded-xl shadow-2xl overflow-hidden">
              <div class="p-2">
                <input
                  v-model="tzSearch"
                  type="text"
              ...
              <div class="max-h-40 overflow-y-auto">
                ...
                <p v-if="filteredTimezones.length === 0" class="px-3.5 py-2.5 text-text-tertiary text-sm">Tidak ditemukan</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="glass rounded-2xl p-5 md:p-6">
        ...
        <p class="text-text-tertiary text-xs mb-4 leading-relaxed">
          Set jam mulai, selesai, dan istirahat default yang akan otomatis terisi saat membuat entri timesheet baru.
        </p>
        <div class="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
          <div>
            <label class="block text-xs font-medium text-text-secondary mb-1.5">Jam Mulai</label>
            <input v-model="form.defaultStartTime" type="time" class="form-input" />
          </div>
          <div>
            <label class="block text-xs font-medium text-text-secondary mb-1.5">Jam Selesai</label>
            <input v-model="form.defaultEndTime" type="time" class="form-input" />
          </div>
          <div class="col-span-2 md:col-span-1">
            <label class="block text-xs font-medium text-text-secondary mb-1.5">Istirahat (menit)</label>
            <input v-model.number="form.defaultBreakMinutes" type="number" min="0" max="480" step="5" class="form-input" />
          </div>
        </div>
        <p class="text-text-tertiary text-[10px] md:text-xs mt-3 leading-snug">
          Durasi kerja akan dikurangi dengan waktu istirahat saat menghitung total jam.
        </p>
      </div>
      ...
      <div class="glass rounded-2xl p-5 md:p-6">
        ...
        <p class="text-text-tertiary text-xs mb-4 leading-relaxed">
          Tambahkan lokasi yang sering digunakan. Jika ada lebih dari 1, lokasi default akan otomatis terpilih saat input timesheet.
        </p>
          ...
          >
            <button
              ...
              :class="[
                ...
                loc.isDefault ? 'border-accent bg-accent' : 'border-white/20 hover:border-accent'
              ]"
              :title="loc.isDefault ? 'Default' : 'Set sebagai default'"
            >
              <svg v-if="loc.isDefault" xmlns="http://www.w3.org/2000/svg" class="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
                <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
              </svg>
            </button>
            <span class="text-sm text-text-primary flex-1 min-w-0 truncate">{{ loc.name }}</span>
            <button
      ...
      <div class="glass rounded-2xl p-5 md:p-6">
        ...
        <p class="text-text-tertiary text-xs mb-4 leading-relaxed">
          Otomatis memparse teks panjang (misal dari Slack wrap-up) menjadi daftar tugas rapi saat paste di input Rincian Tugas.
        </p>

        <div class="space-y-3" :class="{ 'opacity-40 pointer-events-none': !form.textFilter.enabled }">
          <div>
            <label class="block text-xs font-medium text-text-secondary mb-1.5">Task Marker</label>
            <input v-model="form.textFilter.taskMarker" type="text" placeholder="Contoh: ###, ##, * [ ]" class="form-input" />
            <p class="text-text-tertiary text-[10px] md:text-xs mt-1 leading-snug">
          ...
          <div class="space-y-3">
            ...
            >
                ...
                <div class="flex items-center gap-2">
                  ...
                  <span
                    :class="[
                      ...
                      cat.display === 'normal' ? 'bg-accent/15 text-accent' : 'bg-text-tertiary/20 text-text-tertiary'
                    ]"
                  >
              ...
              <div class="mb-2">
                <label class="block text-[10px] font-medium text-text-tertiary mb-1 uppercase tracking-wide">Output Template</label>
                <input v-model="cat.outputTemplate" type="text" placeholder="- {task}" class="form-input text-sm" />
              </div>

              <div class="mb-2">
                <label class="block text-[10px] font-medium text-text-tertiary mb-1 uppercase tracking-wide">Keywords</label>
                <div class="flex flex-wrap gap-1.5 mb-1.5">
                  ...
                  >
                    {{ kw }}
                    <button type="button" @click="removeKeyword(cIdx, kIdx)" class="text-danger hover:text-danger/80 transition-colors">
              ...
              <div>
                <label class="block text-[10px] font-medium text-text-tertiary mb-1 uppercase tracking-wide">Display Style</label>
                <div class="flex gap-2">
                  <button
                    type="button"
      ...
      <div class="glass rounded-2xl p-5 md:p-6">
        ...
        <p class="text-text-tertiary text-xs mb-4 leading-relaxed">
          Coba paste teks wrap-up di bawah untuk melihat hasil parse sebelum disimpan.
        </p>

        <div class="space-y-3">
          <div>
            <label class="block text-xs font-medium text-text-secondary mb-1.5">Raw Text</label>
            <textarea
          ...
          <div class="flex gap-2">
            ...
            <button
              @click="playgroundRaw = ''; playgroundResult = ''"
              type="button"
          ...
          <div v-if="playgroundResult">
            <label class="block text-xs font-medium text-text-secondary mb-1.5">Hasil ({{ playgroundTaskCount }} task)</label>
            <div class="glass rounded-xl p-3 text-sm font-mono text-text-secondary whitespace-pre-wrap leading-relaxed">
      ...
      <div class="glass rounded-2xl p-5 md:p-6">
        ...
        <div class="flex items-center gap-2 mb-3">
          ...
          <span class="text-sm font-medium" :class="isConnected ? 'text-success' : 'text-danger'">
            {{ isConnected ? 'Terhubung' : 'Belum terhubung' }}
          </span>
        </div>

        <p class="text-text-tertiary text-xs mb-4 leading-relaxed">
          Set folder Google Drive tempat evidence disimpan. File akan disimpan dalam subfolder per bulan.
        </p>
        ...
        <p v-if="folderMessage" class="mt-2 text-xs" :class="folderSuccess ? 'text-success' : 'text-danger'">
          {{ folderMessage }}
        </p>
      </div>
      ...
      <div class="glass rounded-2xl p-5 md:p-6">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-text-tertiary text-xs mt-1">Reminder harian untuk mengisi timesheet</p>
          </div>
          <button
      ...
      <div class="glass rounded-2xl p-5 md:p-6 space-y-4">
        <div>
          <h2 class="text-sm md:text-base font-semibold text-text-primary flex items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M9 17v-2m3 2v-4m3 4v-6m2 10h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
            </svg>
            Format Export Excel
          </h2>
          <p class="text-text-tertiary text-xs mt-1">Upload template .xlsx untuk mengganti styling default export</p>
        </div>

        <div v-if="templateLoading" class="flex items-center gap-2 text-text-tertiary text-xs">
        ...
        <div v-else-if="template" class="flex items-center justify-between p-3 glass rounded-xl">
          <div class="flex items-center gap-2 min-w-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-accent flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
            <span class="text-sm text-text-primary truncate">{{ template.fileName }}</span>
          </div>
          <div class="flex items-center gap-1 flex-shrink-0">
            ...
            >
              <svg v-if="templateDeleting" class="w-4 h-4 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
              </svg>
        ...
        >
          ...
          <p class="text-text-secondary text-sm">Klik untuk upload template .xlsx</p>
          <p class="text-text-tertiary text-xs mt-0.5">File template akan mengganti styling default export</p>
          <input
            ref="templateInput"
            type="file"
            accept=".xlsx,.xls"
        ...
        <div v-if="templateUploading" class="flex items-center gap-2 text-text-tertiary text-xs">
          <svg class="w-4 h-4 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
        ...
        <div class="text-text-tertiary text-[10px] leading-relaxed">
          <p>Tips: Template .xlsx dengan styling (warna, font, border) akan dipreserve saat export. Buat template dengan:</p>
        ...
        <div class="glass rounded-2xl p-5 md:p-6 space-y-4">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-text-tertiary text-xs mt-1">Tambahkan alasan seperti Sakit, Izin, Cuti, dll.</p>
            </div>
            <button
          ...
          <div class="flex gap-2">
            <input v-model="newReasonName" type="text" placeholder="Nama alasan..." class="form-input flex-1 text-sm" />
            <input v-model="newReasonColor" type="color" class="w-10 h-9 rounded-lg border border-white/10 bg-transparent cursor-pointer" />
          </div>

          <div v-if="absenceReasons.length === 0" class="text-text-tertiary text-xs text-center py-3">
          ...
          <div v-else class="flex flex-wrap gap-2">
            ...
            >
              <span class="w-2.5 h-2.5 rounded-full flex-shrink-0" :style="{ backgroundColor: reason.color }"></span>
        ...
        <div class="glass rounded-2xl p-5 md:p-6 space-y-4">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-text-tertiary text-xs mt-1">Sinkronisasi dari API libur.deno.dev</p>
            </div>
            <button
              @click="syncHolidays"
            ...
            >
              <svg v-if="syncingHolidays" class="w-3.5 h-3.5 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
              </svg>
          ...
          <div v-if="holidayMessage" class="p-2.5 glass rounded-lg text-xs" :class="holidayMessageType === 'success' ? 'text-accent' : 'text-danger'">
            {{ holidayMessage }}
          </div>

          <div v-if="holidays.length === 0" class="text-text-tertiary text-xs text-center py-3">
            Belum ada data hari libur. Klik "Sync dari API" untuk mengambil data.
          </div>
          ...
          <div v-else class="space-y-1.5 max-h-80 overflow-y-auto pr-1">
            ...
            >
              <div class="flex items-center gap-2 min-w-0">
                <span class="text-xs font-mono text-text-tertiary w-24 flex-shrink-0">{{ h.date }}</span>
                <span class="text-sm text-text-primary truncate">{{ h.name }}</span>
                <span
                  v-if="h.isNationalHoliday"
                  class="px-1.5 py-0.5 rounded text-[10px] bg-accent/10 text-accent flex-shrink-0"
                >Nasional</span>
                <span v-else class="px-1.5 py-0.5 rounded text-[10px] bg-text-tertiary/10 text-text-tertiary flex-shrink-0">Cuti</span>
              </div>
              <button
    ...
    >
      ...
      >
        <div class="glass-strong rounded-2xl p-3 shadow-2xl flex items-center gap-3">
          <div class="flex-1 min-w-0">
            <p class="text-xs text-text-secondary truncate">Ada perubahan yang belum disimpan</p>
          </div>
...
const selectedTzLabel = computed(() => {
  return timezones.find(tz => tz.value === form.value.timezone)?.label || form.value.timezone
})
  ...
  if (user) {
    ...
    if (tf && typeof tf === 'object') {
      form.value.textFilter = {
        ...
        categories: (tf.categories || defaultTextFilter.categories).map(c => ({
          name: c.name,
          keywords: [...(c.keywords || [])],
...
async function verifyFolder() {
  ...
  try {
    ...
    authStore.user = { ...authStore.user, googleDriveFolderId: form.value.googleDriveFolderId }
  } catch (err) {
    folderMessage.value = err.response?.data?.message || 'Gagal memverifikasi folder'
...
async function handleTemplateUpload(event) {
  ...
  try {
    ...
    folderMessage.value = err.response?.data?.message || 'Gagal upload template'
    folderSuccess.value = false
  } finally {
...
async function downloadTemplate() {
  try {
    ...
    folderMessage.value = err.response?.data?.message || 'Gagal download template'
    folderSuccess.value = false
  }
}
...
async function addAbsenceReason() {
  ...
  try {
    ...
    folderMessage.value = err.response?.data?.message || 'Gagal menambah alasan'
    folderSuccess.value = false
  }
}
...
async function removeAbsenceReason(id) {
  try {
    ...
    folderMessage.value = err.response?.data?.message || 'Gagal menghapus alasan'
    folderSuccess.value = false
  }
}
