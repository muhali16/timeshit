<template>
  <div class="min-h-screen text-text-primary pb-20 md:pb-0">
    <!-- Mobile Header -->
    <header class="md:hidden sticky top-0 z-40 glass border-b-0">
      <div class="max-w-4xl mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <div class="relative" ref="profileDropdown">
          ...
          >
            ...
            >
              <span class="text-accent font-bold text-xs">{{ (authStore.user?.name || '?')[0]?.toUpperCase() }}</span>
            </div>
          </button>
          ...
          >
            <div class="px-3.5 py-2.5 border-b border-white/10">
              <p class="text-sm font-medium text-text-primary truncate">{{ authStore.user?.name }}</p>
    ...
    >
      <div class="max-w-4xl mx-auto flex items-center gap-2">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-warning flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" />
        </svg>
        <p class="text-xs text-warning">
          Google Drive folder belum diatur.
          <router-link to="/settings" class="underline font-semibold">Atur di Settings</router-link>
        </p>
      </div>
    </div>
    ...
    <div class="hidden md:flex min-h-screen">
      <!-- Floating Sidebar -->
      <aside class="w-[72px] xl:w-20 flex-shrink-0 p-4 flex flex-col items-center gap-4 fixed h-screen z-40">
        <!-- Logo Pill -->
        <div class="w-full aspect-square rounded-2xl glass-strong flex flex-col items-center justify-center gap-1 animate-glow-pulse">
          <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-accent to-accent-hover flex items-center justify-center shadow-lg shadow-accent/30">
        ...
        <nav class="flex flex-col items-center gap-2 w-full flex-1 py-2">
          <router-link
            ...
            :title="item.label"
          >
            <component :is="item.icon" class="w-5 h-5 transition-transform duration-300 group-hover:scale-110" :active="$route.path === item.path" />
            <span class="text-[10px] font-medium leading-none whitespace-nowrap">{{ item.shortLabel }}</span>
            <!-- Active indicator glow -->
            <div v-if="$route.path === item.path" class="absolute inset-0 rounded-2xl ring-2 ring-accent/30 pointer-events-none"></div>
          </router-link>
        </nav>
        ...
        <div class="w-full flex flex-col items-center gap-2">
          ...
          >
            ...
            <span class="text-[9px] font-medium leading-none whitespace-nowrap">Keluar</span>
          </button>
        </div>
      </aside>

      <!-- Desktop Content Area -->
      <main class="flex-1 ml-[72px] xl:ml-20 p-6 lg:p-8 xl:p-10 overflow-y-auto min-h-screen">
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-warning flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" />
          </svg>
          <p class="text-sm text-warning">
            Google Drive folder belum diatur.
            <router-link to="/settings" class="underline font-semibold hover:text-text-primary transition-colors">Atur di Settings</router-link>
          </p>
        </div>
    ...
    <nav class="md:hidden fixed bottom-0 inset-x-0 z-40 glass border-t-0">
...
const DashboardIcon = { props: ['active'], render() { return h('svg', { xmlns: 'http://www.w3.org/2000/svg', class: this.active ? 'w-5 h-5 text-accent' : 'w-5 h-5', fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': 2 }, [
...
const InputIcon = { props: ['active'], render() { return h('svg', { xmlns: 'http://www.w3.org/2000/svg', class: this.active ? 'w-5 h-5 text-accent' : 'w-5 h-5', fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': 2 }, [
  h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'M12 4v16m8-8H4' })
]) } }

const HistoryIcon = { props: ['active'], render() { return h('svg', { xmlns: 'http://www.w3.org/2000/svg', class: this.active ? 'w-5 h-5 text-accent' : 'w-5 h-5', fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': 2 }, [
  h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z' })
]) } }

const SettingsIcon = { props: ['active'], render() { return h('svg', { xmlns: 'http://www.w3.org/2000/svg', class: this.active ? 'w-5 h-5 text-accent' : 'w-5 h-5', fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': 2 }, [
...
const navItems = [
  ...
  { path: '/history', label: 'Riwayat', shortLabel: 'History', icon: HistoryIcon },
]
